...
class GeotificationHandler: NSObject, Codable, MKAnnotation {
...
extension GeotificationHandler {
  public class func allGeotifications() -> [GeotificationHandler] {
    guard let savedData = UserDefaults.standard.data(forKey: PreferencesKeys.savedItems.rawValue) else { return [] }
    let decoder = JSONDecoder()
    if let savedGeotifications = try? decoder.decode(Array.self, from: savedData) as [GeotificationHandler] {
